...
if __name__ == "__main__":
    ...
    try:
        ...
        if res1 and len(res1) == 3 and type(res1[0]) == str and type(res1[1]) in [int, float] and type(res1[2]) == bool:
            print("<3 Practice 1 Passed! You created all the correct variable types!")
            passed += 1
        else:
    ...
    try:
        res2 = practice_two()
        if res2 == "Hello World":
            print("<3 Practice 2 Passed! You made a string correctly!")
            passed += 1
        else:
            print(f":( Practice 2 Failed. Make sure you return exactly 'Hello World'. Got '{res2}'")
    except Exception as e:
         print(f"o_O Practice 2 Error: {e}")
